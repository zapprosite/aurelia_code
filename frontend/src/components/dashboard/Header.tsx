...
export function Header({ title }: HeaderProps) {
  ...
  const vramText =
    m.vramUsed !== null && m.vramTotal !== null
      ? `${m.vramUsed.toFixed(1)} / ${m.vramTotal.toFixed(1)} GiB`
      : "N/A";

  const gpuText =
    m.gpuUtil !== null ? `${Math.round(m.gpuUtil)}%` : "N/A";
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="hidden md:flex items-center gap-4 px-4 py-1.5 rounded-full bg-white/5 border border-white/5 text-[11px] font-mono text-white/40">
          <div className="flex items-center gap-2">
            ...
            <span>VRAM: <span className="text-white/70">{vramText}</span></span>
          </div>
          <div className="h-3 w-px bg-white/10" />
          <div className="flex items-center gap-2">
             <div className="w-1.5 h-1.5 rounded-full bg-blue-400 shadow-[0_0_5px_rgba(96,165,250,0.5)]" />
             <span>GPU: <span className="text-white/70">{gpuText}</span></span>
          </div>
        </div>
