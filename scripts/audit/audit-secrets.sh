...
PATTERNS=(
    ...
    # GitHub tokens
    "{{GITHUB_TOKEN_HIDDEN}}[A-Za-z0-9_]{36,}"
    "github[_-]?token['\"]?\s*[:=]"
...
EXCLUDE_PATTERNS=(
    ...
    "secret-check.log"  # Rotation log with var names
    "homelab-bibliotheca/" # External skills templates
    ".bak"
    ".backup"
)
...
if [ -d "$HOME/.aurelia/logs" ]; then
    for logfile in "$HOME/.aurelia/logs"/*.log; do
        if [ -f "$logfile" ]; then
            # Check if logfile should be excluded
            if [[ "$(basename "$logfile")" =~ ^($EXCLUDE_REGEX)$ ]]; then
                continue
            fi
...
ENV_FILES=$(find "$REPO_ROOT" \
    ...
    -path "$REPO_ROOT/homelab-bibliotheca" -prune -o \
    \( -name ".env*" -o -name "*.env" \) \
    -not -name "secrets.env" -not -name ".env.example" -print 2>/dev/null)
