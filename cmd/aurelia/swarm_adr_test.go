...
// TestTeamExecutaADR simula o humano enviando um prompt desafiador pelo Telegram
...
// (ou equivalente rápido) para as inferências do time no .env local.
func TestTeamExecutaADR(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Erro crítico ao subir o ambiente Team Runtime: %v", err)
	}
	defer app.close()
	...
	// 3. O Desafio (Prompt)
	prompt := "Aurelia, aqui é o humano. Eu te desafio a executar agora mesmo a alteração/aplicação descrita na 'ADR de mudança de modelos locais' (ADR-20260320-politica-modelos-hardware-vram.md). Confio na velocidade do modelo configurado (MiniMax 3.7/Claude 3.7). Trabalhe com o time em tempo recorde e me dê o resultado."

	t.Logf("==== INICIANDO INJEÇÃO DO PROMPT PARA O TIME ====")
	...
	err = app.primaryBot.ProcessExternalInput(context.Background(), adminID, chatID, prompt, false)
	if err != nil {
		t.Fatalf("Falha do bot ao processar o input: %v", err)
	}

	// 5. Como o time roda em background criando rotinas de loop e agentes (Lead -> Planner -> Child),
	// nós seguramos o teste aberto para observar o console enquanto eles "trabalham".
	t.Log("O prompt foi aceito pelo Telegram Controller e despachado pro Loop de Team Orchestration.")
	...
	t.Log("==== TESTE E2E DO TIME FINALIZADO ====")
}

// TestTeamImplementaP3 simula o humano enviando o desafio do Roadmap Mestre P3
// para a Aurelia, mandando o time fazer o código e provar habilidades Híbridas.
func TestTeamImplementaP3(t *testing.T) {
	if os.Getenv("RUN_SWARM_E2E") == "" {
	...
	prompt := "Aurelia/MiniMax, aqui é o humano te desafiando: Implemente agora a Feature [P3] listada no ADR-20260320-roadmap-mestre-slices.md. Faça o cherry-pick dos arquivos 'scripts/simulate_swarm_2026.go' e da pasta 'internal/voice' da branch agent-to-agent. Mostre que o time pode usar todas as skills híbridas recém unificadas e feche isso em tempo recorde!"

	t.Logf("==== INJEÇÃO DO PROMPT DE IMPLEMENTAÇÃO P3 TEAM ORCHESTRATION ====")
	t.Logf("Prompt: %s", prompt)
	t.Logf("==========================================================")
	...
	t.Log("O prompt foi injetado na mente do Team Runtime (Loop Agentico).")
	t.Log("Aguardando 45s de processamento MiniMax...")
	time.Sleep(45 * time.Second)

	app.shutdown(context.Background())
	t.Log("==== TESTE P3 TEAM ORCHESTRATION FINALIZADO ====")
}
